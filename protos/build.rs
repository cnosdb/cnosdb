...
",
    ...
    // build .proto files
    {
        ...
        let output_dir_final =
            env::current_dir().unwrap().join("src").join("generated").join("protobuf_generated");
        ...
        tonic_build::configure().out_dir(&output_dir_final)
    ...
    // build .fbs files
    {
        ...
        let output_dir_final =
            env::current_dir().unwrap().join("src").join("generated").join("flatbuffers_generated");
        fs::create_dir_all(&output_dir_final)?;
        ...
        for p in fbs_file_paths.iter() {
            let output_rust_mod_name = p.file_name()
                                        .unwrap()
                                        .to_str()
                                        .unwrap()
                                        .split('.')
            ...
            let output =
                Command::new("flatc").arg("-o")
                                     ...
                                     .arg(p)
                                     .output()
                                     .expect(&*format!("Failed to generate file by flatbuffers {}.",
                                                       output_file_name));
            ...
            Command::new("rustfmt").arg(output_file_path).output().expect("Failed to format file.");
        }
    }
