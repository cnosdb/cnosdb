...
func NewVerify(concurrent int, logger *zap.Logger) *Verify {
	return &Verify{
...
func (v Verify) VerifySeriesFile(filePath string) (valid bool, err error) {
	...
	defer func() {
		if rec := recover(); rec != nil {
			v.Logger.Error("Panic verifying seriesFile", zap.String("recovered", fmt.Sprint(rec)))
			valid = false
		}
	}()
	...
	if os.IsNotExist(err) {
		v.Logger.Error("Series seriesFile does not exist")
		return false, nil
	}
	if err != nil {
		return false, err
