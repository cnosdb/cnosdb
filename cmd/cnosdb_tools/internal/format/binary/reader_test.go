...
func TestReader_OneBucketOneIntegerSeries(t *testing.T) {
	...
	bw.BeginSeries(s.seriesName, s.seriesField, cnosql.Integer, s.seriesTags)
...
func TestReader_OneBucketOneFloatSeries(t *testing.T) {
	...
	w := binary.NewWriter(&buf, s.db, s.rp, s.sd)
	bw, _ := w.NewBucket(s.start, s.end)
	bw.BeginSeries(s.seriesName, s.seriesField, cnosql.Float, s.seriesTags)
...
func TestReader_OneBucketOneUnsignedSeries(t *testing.T) {
	...
	w := binary.NewWriter(&buf, s.db, s.rp, s.sd)
	bw, _ := w.NewBucket(s.start, s.end)
	bw.BeginSeries(s.seriesName, s.seriesField, cnosql.Unsigned, s.seriesTags)
...
func TestReader_OneBucketOneBooleanSeries(t *testing.T) {
	...
	w := binary.NewWriter(&buf, s.db, s.rp, s.sd)
	bw, _ := w.NewBucket(s.start, s.end)
	bw.BeginSeries(s.seriesName, s.seriesField, cnosql.Boolean, s.seriesTags)
...
func TestReader_OneBucketOneStringSeries(t *testing.T) {
	...
	w := binary.NewWriter(&buf, s.db, s.rp, s.sd)
	bw, _ := w.NewBucket(s.start, s.end)
	bw.BeginSeries(s.seriesName, s.seriesField, cnosql.String, s.seriesTags)
...
func TestReader_OneBucketMixedSeries(t *testing.T) {
	...
	bw.BeginSeries(seriesName, seriesField, cnosql.Integer, seriesTags1)
	t1s := []int64{0, 1, 2}
	v1s := []int64{10, 11, 12}
	bw.WriteIntegerCursor(&intCursor{1, t1s, v1s})
	bw.EndSeries()
	bw.BeginSeries(seriesName, seriesField, cnosql.Integer, seriesTags2)
	t2s := []int64{1, 2, 3}
	v2s := []float64{7, 8, 9}
