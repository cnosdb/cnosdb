...
func ResultSetToLineProtocol(wr io.Writer, rs ResultSet) (err error) {
	...
	for rs.Next() {
		tags := rs.Tags()
		name := tags.Get(models.MetricTagKeyBytes)
		field := tags.Get(models.FieldKeyTagKeyBytes)
		if len(name) == 0 || len(field) == 0 {
			return errors.New("missing metric / field")
		}

		line = append(line[:0], name...)
		if tags.Len() > 2 {
			tags = tags[1 : len(tags)-1] // take first and last elements which are metric and field keys
