...
func NewSeriesGeneratorFromSpec(s *Spec, tr TimeRange) SeriesGenerator {
	sg := make([]SeriesGenerator, len(s.Metrics))
	for i := range s.Metrics {
		sg[i] = newSeriesGeneratorFromMetricSpec(&s.Metrics[i], tr)
	}
...
func newSeriesGeneratorFromMetricSpec(ms *MetricSpec, tr TimeRange) SeriesGenerator {
...
func (s *schemaToSpec) visit(node SchemaNode) bool {
	switch n := node.(type) {
	case *Schema:
		s.spec.Metrics = s.pop().([]MetricSpec)
	...
	case Metrics:
		...
		var mss []MetricSpec
		for {
			if specs, ok := s.peek().([]MetricSpec); ok {
				s.pop()
		...
		// validate field types are homogeneous for a single metric
	...
	case *Metric:
		if len(n.Name) == 0 {
			s.err = errors.New("missing metric name")
			return false
		}
		...
		// NOTE: sort each metric name + field name to ensure series are produced
		//  in correct order
...
func (s *schemaToSpec) visitErr(node SchemaNode) {
	...
	case *Metric:
		s.err = fmt.Errorf("metric %q: %v", n.Name, s.err)
	case *Tag:
