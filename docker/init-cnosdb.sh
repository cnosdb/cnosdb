...
init_cnosdb() {
  if [ -z "$AUTH_ENABLED" ]; then
  	AUTH_ENABLED="$(grep -iE '^s*auth-enableds*=s*true' /etc/cnosdb/cnosdb.conf | grep -io 'true' | cat)"
  else
...
init_cnosdb_meta() {
  if [ -z "$AUTH_ENABLED" ]; then
    	AUTH_ENABLED="$(grep -iE '^s*auth-enableds*=s*true' /etc/cnosdb/cnosdb-meta.conf | grep -io 'true' | cat)"
    else
    	AUTH_ENABLED="$(echo ""$CNOSDB_HTTP_AUTH_ENABLED"" | grep -io 'true' | cat)"
    fi
  ## TODO
...
init_cnosdb_data() {
  if [ -z "$AUTH_ENABLED" ]; then
    	AUTH_ENABLED="$(grep -iE '^s*auth-enableds*=s*true' /etc/cnosdb/cnosdb-data.conf | grep -io 'true' | cat)"
    else
    	AUTH_ENABLED="$(echo ""$CNOSDB_HTTP_AUTH_ENABLED"" | grep -io 'true' | cat)"
    fi
  ## TODO
