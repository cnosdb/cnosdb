...
func exprEqual(x, y cnosql.Expr) bool {
	if x == nil {
		if y == nil {
			return true
		}
...
func TestSeriesCursorValuer(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.n, func(t *testing.T) {
			...
			expr, err := cnosql.ParseExpr(tc.expr)
			if err != nil {
				t.Fatalf("unable to parse input expression %q, %v", tc.expr, err)
			}
			exp, err := cnosql.ParseExpr(tc.exp)
			if err != nil {
				t.Fatalf("unable to parse expected expression %q, %v", tc.exp, err)
			}

			if got := cnosql.Reduce(expr, &sc); !cmp.Equal(got, exp, cmp.Comparer(exprEqual)) {
