...
type StatementExecutor struct {
	ExecuteStatementFn func(stmt cnosql.Statement, ctx *query.ExecutionContext) error
}
...
func TestQueryExecutor_AttachQuery(t *testing.T) {
	q, err := cnosql.ParseQuery(`SELECT count(value) FROM air`)
	if err != nil {
		t.Fatal(err)
	}
...
func TestQueryExecutor_KillQuery(t *testing.T) {
	...
	q, err = cnosql.ParseQuery(fmt.Sprintf("KILL QUERY %d", <-qid))
	if err != nil {
		t.Fatal(err)
	}
	discardOutput(e.ExecuteQuery(q, query.ExecutionOptions{}, nil))
...
func TestQueryExecutor_KillQuery_Zombie(t *testing.T) {
	...
	e.StatementExecutor = &StatementExecutor{
		ExecuteStatementFn: func(stmt cnosql.Statement, ctx *query.ExecutionContext) error {
			switch stmt.(type) {
			case *cnosql.KillQueryStatement, *cnosql.ShowQueriesStatement:
				return e.TaskManager.ExecuteStatement(ctx, stmt)
			}
	...
	q, err = cnosql.ParseQuery("SHOW QUERIES")
	if err != nil {
		t.Fatal(err)
	}
...
func TestQueryExecutor_Panic(t *testing.T) {
	...
	if result.Err == nil || result.Err.Error() != "SELECT count(value) FROM air [panic:test error]" {
		t.Errorf("unexpected error: %s", result.Err)
	}
}
