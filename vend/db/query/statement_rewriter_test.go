...
func TestRewriteStatement(t *testing.T) {
	tests := []struct {
		...
		{
			stmt: `SHOW FIELD KEYS FROM air`,
			s:    `SELECT fieldKey, fieldType FROM _fieldKeys WHERE _name = 'air'`,
		},
		{
			stmt: `SHOW FIELD KEYS ON db0 FROM air`,
			s:    `SELECT fieldKey, fieldType FROM db0.._fieldKeys WHERE _name = 'air'`,
		},
		{
		...
		{
			stmt: `SHOW FIELD KEYS FROM mydb.myrp2.air`,
			s:    `SELECT fieldKey, fieldType FROM mydb.myrp2._fieldKeys WHERE _name = 'air'`,
		},
		{
			stmt: `SHOW FIELD KEYS ON db0 FROM mydb.myrp2.air`,
			s:    `SELECT fieldKey, fieldType FROM mydb.myrp2._fieldKeys WHERE _name = 'air'`,
		},
		{
		...
		{
			stmt: `SHOW SERIES FROM air`,
			s:    `SELECT "key" FROM _series WHERE _name = 'air'`,
		},
		{
			stmt: `SHOW SERIES ON db0 FROM air`,
			s:    `SELECT "key" FROM db0.._series WHERE _name = 'air'`,
		},
		{
			stmt: `SHOW SERIES FROM mydb.myrp1.air`,
			s:    `SELECT "key" FROM mydb.myrp1._series WHERE _name = 'air'`,
		},
		{
			stmt: `SHOW SERIES ON db0 FROM mydb.myrp1.air`,
			s:    `SELECT "key" FROM mydb.myrp1._series WHERE _name = 'air'`,
		},
		{
		...
		{
			stmt: `SHOW SERIES FROM air WHERE time > 0`,
			s:    `SELECT _seriesKey AS "key" FROM air WHERE time > 0`,
		},
		{
			stmt: `SHOW SERIES ON db0 FROM air WHERE time > 0`,
			s:    `SELECT _seriesKey AS "key" FROM db0..air WHERE time > 0`,
		},
		{
			stmt: `SHOW SERIES FROM mydb.myrp1.air WHERE time > 0`,
			s:    `SELECT _seriesKey AS "key" FROM mydb.myrp1.air WHERE time > 0`,
		},
		{
			stmt: `SHOW SERIES ON db0 FROM mydb.myrp1.air WHERE time > 0`,
			s:    `SELECT _seriesKey AS "key" FROM mydb.myrp1.air WHERE time > 0`,
		},
		{
		...
		{
			stmt: `SHOW TAG KEYS FROM air`,
			s:    `SHOW TAG KEYS WHERE _name = 'air'`,
		},
		{
			stmt: `SHOW TAG KEYS ON db0 FROM air`,
			s:    `SHOW TAG KEYS ON db0 WHERE _name = 'air'`,
		},
		{
		...
		{
			stmt: `SHOW TAG KEYS FROM air WHERE region = 'uswest'`,
			s:    `SHOW TAG KEYS WHERE (_name = 'air') AND (region = 'uswest')`,
		},
		{
			stmt: `SHOW TAG KEYS ON db0 FROM air WHERE region = 'uswest'`,
			s:    `SHOW TAG KEYS ON db0 WHERE (_name = 'air') AND (region = 'uswest')`,
		},
		{
			stmt: `SHOW TAG KEYS FROM mydb.myrp1.air`,
			s:    `SHOW TAG KEYS WHERE _name = 'air'`,
		},
		{
			stmt: `SHOW TAG KEYS ON db0 FROM mydb.myrp1.air`,
			s:    `SHOW TAG KEYS ON db0 WHERE _name = 'air'`,
		},
		{
		...
		{
			stmt: `SHOW TAG KEYS FROM mydb.myrp1.air WHERE region = 'uswest'`,
			s:    `SHOW TAG KEYS WHERE (_name = 'air') AND (region = 'uswest')`,
		},
		{
			stmt: `SHOW TAG KEYS ON db0 FROM mydb.myrp1.air WHERE region = 'uswest'`,
			s:    `SHOW TAG KEYS ON db0 WHERE (_name = 'air') AND (region = 'uswest')`,
		},
		{
		...
		{
			stmt: `SHOW TAG KEYS FROM air WHERE time > 0`,
			s:    `SHOW TAG KEYS WHERE (_name = 'air') AND (time > 0)`,
		},
		{
			stmt: `SHOW TAG KEYS ON db0 FROM air WHERE time > 0`,
			s:    `SHOW TAG KEYS ON db0 WHERE (_name = 'air') AND (time > 0)`,
		},
		{
		...
		{
			stmt: `SHOW TAG KEYS FROM air WHERE region = 'uswest' AND time > 0`,
			s:    `SHOW TAG KEYS WHERE (_name = 'air') AND (region = 'uswest' AND time > 0)`,
		},
		{
			stmt: `SHOW TAG KEYS ON db0 FROM air WHERE region = 'uswest' AND time > 0`,
			s:    `SHOW TAG KEYS ON db0 WHERE (_name = 'air') AND (region = 'uswest' AND time > 0)`,
		},
		{
			stmt: `SHOW TAG KEYS FROM mydb.myrp1.air WHERE time > 0`,
			s:    `SHOW TAG KEYS WHERE (_name = 'air') AND (time > 0)`,
		},
		{
			stmt: `SHOW TAG KEYS ON db0 FROM mydb.myrp1.air WHERE time > 0`,
			s:    `SHOW TAG KEYS ON db0 WHERE (_name = 'air') AND (time > 0)`,
		},
		{
		...
		{
			stmt: `SHOW TAG KEYS FROM mydb.myrp1.air WHERE region = 'uswest' AND time > 0`,
			s:    `SHOW TAG KEYS WHERE (_name = 'air') AND (region = 'uswest' AND time > 0)`,
		},
		{
			stmt: `SHOW TAG KEYS ON db0 FROM mydb.myrp1.air WHERE region = 'uswest' AND time > 0`,
			s:    `SHOW TAG KEYS ON db0 WHERE (_name = 'air') AND (region = 'uswest' AND time > 0)`,
		},
		{
		...
		{
			stmt: `SHOW TAG VALUES FROM air WITH KEY = "region"`,
			s:    `SHOW TAG VALUES WITH KEY = region WHERE (_name = 'air') AND (_tagKey = 'region')`,
		},
		{
			stmt: `SHOW TAG VALUES WITH KEY != "region"`,
			s:    `SHOW TAG VALUES WITH KEY != region WHERE _tagKey != 'region'`,
		...
		{
			stmt: `SELECT value FROM air`,
			s:    `SELECT value FROM air`,
		},
	}

	for _, test := range tests {
		t.Run(test.stmt, func(t *testing.T) {
			stmt, err := cnosql.ParseStatement(test.stmt)
			if err != nil {
