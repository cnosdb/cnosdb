...
func TestMath_TypeMapper(t *testing.T) {
	for _, tt := range []struct {
		...
		{s: `abs(f::float)`, typ: cnosql.Float},
		{s: `abs(i::integer)`, typ: cnosql.Integer},
		{s: `abs(u::unsigned)`, typ: cnosql.Unsigned},
		{s: `abs(s::string)`, err: true},
		{s: `abs(b::boolean)`, err: true},
		{s: `sin(f::float)`, typ: cnosql.Float},
		{s: `sin(i::integer)`, typ: cnosql.Float},
		{s: `sin(u::unsigned)`, typ: cnosql.Float},
		{s: `sin(s::string)`, err: true},
		{s: `sin(b::boolean)`, err: true},
		{s: `cos(f::float)`, typ: cnosql.Float},
		{s: `cos(i::integer)`, typ: cnosql.Float},
		{s: `cos(u::unsigned)`, typ: cnosql.Float},
		{s: `cos(s::string)`, err: true},
		{s: `cos(b::boolean)`, err: true},
		{s: `tan(f::float)`, typ: cnosql.Float},
		{s: `tan(i::integer)`, typ: cnosql.Float},
		{s: `tan(u::unsigned)`, typ: cnosql.Float},
		{s: `tan(s::string)`, err: true},
		{s: `tan(b::boolean)`, err: true},
		{s: `asin(f::float)`, typ: cnosql.Float},
		{s: `asin(i::integer)`, err: true},
		{s: `asin(u::unsigned)`, err: true},
		{s: `asin(s::string)`, err: true},
		{s: `asin(b::boolean)`, err: true},
		{s: `acos(f::float)`, typ: cnosql.Float},
		{s: `acos(i::integer)`, err: true},
		{s: `acos(u::unsigned)`, err: true},
		{s: `acos(s::string)`, err: true},
		{s: `acos(b::boolean)`, err: true},
		{s: `atan(f::float)`, typ: cnosql.Float},
		{s: `atan(i::integer)`, typ: cnosql.Float},
		{s: `atan(u::unsigned)`, typ: cnosql.Float},
		{s: `atan(s::string)`, err: true},
		{s: `atan(b::boolean)`, err: true},
		{s: `atan2(y::float, x::float)`, typ: cnosql.Float},
		{s: `atan2(y::integer, x::float)`, typ: cnosql.Float},
		{s: `atan2(y::unsigned, x::float)`, typ: cnosql.Float},
		{s: `atan2(y::string, x::float)`, err: true},
		{s: `atan2(y::boolean, x::float)`, err: true},
		{s: `atan2(y::float, x::float)`, typ: cnosql.Float},
		{s: `atan2(y::float, x::integer)`, typ: cnosql.Float},
		{s: `atan2(y::float, x::unsigned)`, typ: cnosql.Float},
		{s: `atan2(y::float, x::string)`, err: true},
		{s: `atan2(y::float, x::boolean)`, err: true},
		{s: `exp(f::float)`, typ: cnosql.Float},
		{s: `exp(i::integer)`, typ: cnosql.Float},
		{s: `exp(u::unsigned)`, typ: cnosql.Float},
		{s: `exp(s::string)`, err: true},
		{s: `exp(b::boolean)`, err: true},
		{s: `log(f::float)`, typ: cnosql.Float},
		{s: `log(i::integer)`, typ: cnosql.Float},
		{s: `log(u::unsigned)`, typ: cnosql.Float},
		{s: `log(s::string)`, err: true},
		{s: `log(b::boolean)`, err: true},
		{s: `ln(f::float)`, typ: cnosql.Float},
		{s: `ln(i::integer)`, typ: cnosql.Float},
		{s: `ln(u::unsigned)`, typ: cnosql.Float},
		{s: `ln(s::string)`, err: true},
		{s: `ln(b::boolean)`, err: true},
		{s: `log2(f::float)`, typ: cnosql.Float},
		{s: `log2(i::integer)`, typ: cnosql.Float},
		{s: `log2(u::unsigned)`, typ: cnosql.Float},
		{s: `log2(s::string)`, err: true},
		{s: `log2(b::boolean)`, err: true},
		{s: `log10(f::float)`, typ: cnosql.Float},
		{s: `log10(i::integer)`, typ: cnosql.Float},
		{s: `log10(u::unsigned)`, typ: cnosql.Float},
		{s: `log10(s::string)`, err: true},
		{s: `log10(b::boolean)`, err: true},
		{s: `sqrt(f::float)`, typ: cnosql.Float},
		{s: `sqrt(i::integer)`, typ: cnosql.Float},
		{s: `sqrt(u::unsigned)`, typ: cnosql.Float},
		{s: `sqrt(s::string)`, err: true},
		{s: `sqrt(b::boolean)`, err: true},
		{s: `pow(y::float, x::float)`, typ: cnosql.Float},
		{s: `pow(y::integer, x::float)`, typ: cnosql.Float},
		{s: `pow(y::unsigned, x::float)`, typ: cnosql.Float},
		{s: `pow(y::string, x::string)`, err: true},
		{s: `pow(y::boolean, x::boolean)`, err: true},
		{s: `pow(y::float, x::float)`, typ: cnosql.Float},
		{s: `pow(y::float, x::integer)`, typ: cnosql.Float},
		{s: `pow(y::float, x::unsigned)`, typ: cnosql.Float},
		{s: `pow(y::float, x::string)`, err: true},
		{s: `pow(y::float, x::boolean)`, err: true},
		{s: `floor(f::float)`, typ: cnosql.Float},
		{s: `floor(i::integer)`, typ: cnosql.Integer},
		{s: `floor(u::unsigned)`, typ: cnosql.Unsigned},
		{s: `floor(s::string)`, err: true},
		{s: `floor(b::boolean)`, err: true},
		{s: `ceil(f::float)`, typ: cnosql.Float},
		{s: `ceil(i::integer)`, typ: cnosql.Integer},
		{s: `ceil(u::unsigned)`, typ: cnosql.Unsigned},
		{s: `ceil(s::string)`, err: true},
		{s: `ceil(b::boolean)`, err: true},
		{s: `round(f::float)`, typ: cnosql.Float},
		{s: `round(i::integer)`, typ: cnosql.Integer},
		{s: `round(u::unsigned)`, typ: cnosql.Unsigned},
