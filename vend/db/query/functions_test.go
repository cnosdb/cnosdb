...
func TestHoltWinters_RoundTime(t *testing.T) {
	maxTime := time.Unix(0, cnosql.MaxTime).Round(time.Second).UnixNano()
...
func TestHoltWinters_MaxTime(t *testing.T) {
	data := []query.FloatPoint{
		{Time: cnosql.MaxTime - 1, Value: 1},
		{Time: cnosql.MaxTime, Value: 2},
	}
	...
	forecasted := []query.FloatPoint{
		{Time: cnosql.MaxTime - 1, Value: 1},
		{Time: cnosql.MaxTime, Value: 2.001516944066403},
		{Time: cnosql.MaxTime + 1, Value: 2.5365248972488343},
	}
