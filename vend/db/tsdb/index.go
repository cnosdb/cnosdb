...
func (itr *seriesPointIterator) Next() (*query.FloatPoint, error) {
	for {
		...
		// TODO: It seems to me like this authorisation check should be
...
func (is IndexSet) measurementNamesByTagFilter(auth query.FineAuthorizer, op cnosql.Token, key, val string, regex *regexp.Regexp) ([][]byte, error) {
	...
	for {
		...
		//     False  |       True      |      False
		//     False  |       False     |      True
		if tagMatch == (op == cnosql.EQ || op == cnosql.EQREGEX) && authorized {
...
// series file.
func (is IndexSet) tagValuesByKeyAndExpr(auth query.FineAuthorizer, name []byte, keys []string, expr cnosql.Expr) ([]map[string]struct{}, error) {
