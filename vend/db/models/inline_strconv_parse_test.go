...
func TestParseUintBytesEquivalenceFuzz(t *testing.T) {
	f := func(b []byte, bitSize int) bool {
		exp, expErr := strconv.ParseUint(string(b), 10, bitSize)
		got, gotErr := parseUintBytes(b, 10, bitSize)
...
func TestParseUintBytesValid64bitEquivalenceFuzz(t *testing.T) {
	buf := []byte{}
	f := func(n uint64) bool {
		buf = strconv.AppendUint(buf[:0], n, 10)

		exp, expErr := strconv.ParseUint(string(buf), 10, 64)
		got, gotErr := parseUintBytes(buf, 10, 64)
