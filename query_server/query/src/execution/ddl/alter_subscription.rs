...
use spi::query::logical_planner::AlterSubscription;
use spi::{QueryError, Result};

use crate::execution::ddl::DDLDefinitionTask;

pub struct AlterSubscriptionTask {
    stmt: AlterSubscription,
}

impl AlterSubscriptionTask {
    pub fn new(stmt: AlterSubscription) -> Self {
        Self { stmt }
    }
}

#[async_trait]
impl DDLDefinitionTask for AlterSubscriptionTask {
    async fn execute(&self, query_state_machine: QueryStateMachineRef) -> Result<Output> {
        ...
        meta.update_subscription(db_name, &subs_info).await?;
        Ok(Output::Nil(()))
    }
}
