...
use datafusion::logical_expr::{aggregate_function, Aggregate, Distinct, LogicalPlan};
...
/// Wrap the `input` of the aggregate plan with a `Distinct` node,
/// and transform UDF definitions of the plan's `expressions`:
...
fn transform_aggregation(aggregate: Aggregate) -> Result<Aggregate> {
    let new_aggr_input = Arc::new(LogicalPlan::Distinct(Distinct {
    ...
    Aggregate::try_new(new_aggr_input, aggregate.group_expr, new_aggr_exprs)
}
