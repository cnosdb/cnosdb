...
CREATE DATABASE show_tag_values;
...
SHOW TAG VALUES FROM test WITH KEY IN (t0, t1, t2) ORDER BY key;
...
SHOW TAG VALUES FROM test WITH KEY IN (t0, t1, t2) WHERE time < now() ORDER BY key;
SHOW TAG VALUES FROM test WITH KEY IN (t0, t1, t2) WHERE f1 IS NOT NULL ORDER BY key;
SHOW TAG VALUES FROM test WITH KEY IN (t0, t1, t2) WHERE t0 != '' ORDER BY key;
SHOW TAG VALUES FROM test WITH KEY IN (t0, t1, t2) WHERE t0 IS NOT NULL ORDER BY key;
SHOW TAG VALUES FROM test WITH KEY IN (t0, t1, t2) WHERE t1 = 'b' ORDER BY key;
