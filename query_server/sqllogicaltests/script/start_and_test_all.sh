...
# test case
function test() {
  echo "Testing query/sqllogicaltests" &&
...
function 1m1d() {
  nohup ${META_PATH} --config ${CONF_META_1} >${MT_1_LOG_PATH} &
  2>1 &
  sleep 3
  rpc 8901/init '{}'
  sleep 1
  rpc 8901/metrics
  nohup ${EXE_PATH} run --config ${CONF_DATA_1} >${DN_1_LOG_PATH} &
  2>1 &
}
function 1m2d_sep() {
  nohup ${META_PATH} --config ${CONF_META_1} >${MT_1_LOG_PATH} &
  ...
  #  sleep 1
  rpc 8901/metrics
  nohup ${EXE_PATH} run -M query -c 2 --config ${CONF_DATA_1} >${DN_1_LOG_PATH} &
  2>1 &
  nohup ${EXE_PATH} run -M tskv -c 2 --config ${CONF_DATA_2} >${DN_2_LOG_PATH} &
  2>1 &
}
...
function 3m2d() {
  nohup ${META_PATH} --config ${CONF_META_1} >${MT_1_LOG_PATH} &
  2>1 &
  nohup ${META_PATH} --config ${CONF_META_2} >${MT_2_LOG_PATH} &
  2>1 &
  nohup ${META_PATH} --config ${CONF_META_3} >${MT_3_LOG_PATH} &
  2>1 &
  sleep 3
  rpc 8901/init '{}'
  sleep 1
  ...
  sleep 1
  rpc 8901/metrics
  nohup ${EXE_PATH} run -c 2 --config ${CONF_DATA_1} >${DN_1_LOG_PATH} &
  2>1 &
  nohup ${EXE_PATH} run -c 2 --config ${CONF_DATA_2} >${DN_2_LOG_PATH} &
  2>1 &
}
function 1m2d() {
  nohup ${META_PATH} --config ${CONF_META_1} >${MT_1_LOG_PATH} &
  ...
  nohup ${EXE_PATH} run --config ${CONF_DATA_1} >${DN_1_LOG_PATH} &
  2>1 &
  nohup ${EXE_PATH} run --config ${CONF_DATA_2} >${DN_2_LOG_PATH} &
  2>1 &
}
function adddatanode() {
  ...
  ./target/test-ci/cnosdb-cli -P 8902 -f ./query_server/sqllogicaltests/script/insert.sql
  # start datanode 3
  nohup ${EXE_PATH} run --config ${CONF_DATA_3} >${DN_3_LOG_PATH} &
  2>1 &
  sleep 3
}
function query() {
  result=$(./target/test-ci/cnosdb-cli -P 8902 -f ./query_server/sqllogicaltests/script/query.sql)
  # check query results
...
while [[ $# -gt 0 ]]; do
  ...
  case ${key} in
  singleton)
    nohup ${EXE_PATH} run -M singleton --config ${CONF_DATA_1} >${DN_1_LOG_PATH} &
    2>1 &
    shift 1
    ;;
  3meta2data)
