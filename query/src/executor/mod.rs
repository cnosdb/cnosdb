// note: executor adapted from influxdb_iox
// https://github.com/influxdata/influxdb_iox/tree/main/executor
...
impl<T> Job<T> {
    ...
    ///
    /// You must ensure that this task eventually finishes, otherwise [`DedicatedExecutor::join`]
    /// may never return!
    pub fn detach(mut self) {
        // cannot destructure `Self` because we implement `Drop`, so we use a flag instead to
        // prevent cancelation.
        self.detached = true;
    }
}
...
// IMPORTANT: Implement `Drop` for `State`, NOT for `DedicatedExecutor`, because the executor can be
// cloned and clones share their inner state.
impl Drop for State {
    fn drop(&mut self) {
...
impl DedicatedExecutor {
    ...
    pub fn new(thread_name: &str, num_threads: usize) -> Self {
        ...
        let state = State { requests: Some(tx_tasks),
                            task_refs: Arc::new(()),
              ...
              T::Output: Send + 'static
    {
        ...
        let task = Task { fut, cancel: cancel.clone(), task_ref: Arc::clone(&state.task_refs) };
    ...
    ///
    /// # Panic / Drop
    /// [`DedicatedExecutor`] implements shutdown on [`Drop`]. You should just use this behavior and
    /// NOT call [`join`](Self::join) manually during [`Drop`] or panics because this might lead
    /// to another panic, see <https://github.com/rust-lang/futures-rs/issues/2575>.
...
mod tests {
    ...
    async fn tokio_spawn() {
        ...
        let dedicated_task = exec.spawn(async move {
                                     ...
                                     let t1 = tokio::task::spawn(async {
                                         assert_eq!(std::thread::current().name(),
    ...
    async fn detach_receiver() {
        ...
        // create first task
        // `detach()` consumes the task but doesn't abort the task (in contrast to `drop`). We'll
        // proof the that the task is still running by linking it to a 2nd task using a
        // barrier with size 3 (two tasks plus the main thread).
