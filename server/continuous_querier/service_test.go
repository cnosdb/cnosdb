...
func TestContinuousQueryService_ResampleOptions(t *testing.T) {
	...
	if err != nil {
		...
		t.Errorf("expected resample every to be 10s, got %s", cnosql.FormatDuration(cq.Resample.Every))
	} else if cq.Resample.For != 2*time.Minute {
		t.Errorf("expected resample for 2m, got %s", cnosql.FormatDuration(cq.Resample.For))
	}
	...
	s.QueryExecutor.StatementExecutor = &StatementExecutor{
		ExecuteStatementFn: func(stmt cnosql.Statement, ctx *query.ExecutionContext) error {
			...
			valuer := &cnosql.NowValuer{Location: s.Location}
			_, timeRange, err := cnosql.ConditionExpr(s.Condition, valuer)
			if err != nil {
...
func TestExecuteContinuousQuery_TimeRange(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.d, func(t *testing.T) {
			d, err := cnosql.ParseDuration(tt.d)
			if err != nil {
