...
This document lays out continuous queries and a proposed architecture for how they'll work within an CnosDB cluster.

## Definition of Continuous Queries

Continuous queries serve two purposes in CnosDB:
...
I think this transform function be something separate from the regular query planner and engine. It can be in `CnosQL` but it should be something fairly simply since the only purpose of these types of queries is to either filter some data out and output to a new series or transform into a new series by dropping tags.
