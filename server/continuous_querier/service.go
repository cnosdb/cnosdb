...
func (s *Service) runContinuousQueryAndWriteResult(cq *ContinuousQuery) *query.Result {
	...
	q := &cnosql.Query{
		Statements: cnosql.Statements([]cnosql.Statement{cq.q}),
	}
...
// NewContinuousQuery returns a ContinuousQuery object with a parsed cnosql.CreateContinuousQueryStatement.
func NewContinuousQuery(database string, cqi *meta.ContinuousQueryInfo) (*ContinuousQuery, error) {
	stmt, err := cnosql.NewParser(strings.NewReader(cqi.Query)).ParseStatement()
	if err != nil {
		return nil, err
	}
