...
# Default-Stop:      0 1 6
# Short-Description: Start the CnosDB process
### END INIT INFO

# If you modify this, please make sure to also edit cnosdb.service

# Command-line options that can be set in /etc/default/cnosdb.  These will override
# any config file values.
DEFAULT=/etc/default/cnosdb-meta
...
# Configuration file
CONFIG=/etc/cnosdb-meta/cnosdb-meta.conf

# PID file for the daemon
PIDFILE=/var/run/cnosdb-meta/cnosdb-meta.pid
...
if [ -z "$STDERR" ]; then
    STDERR=/var/log/cnosdb-meta/cnosdb-meta.log
fi
...
function start() {
    ...
    if command -v start-stop-daemon &>/dev/null; then
        start-stop-daemon \
            ...
            -pidfile $PIDFILE \
            -config $CONFIG \
            $CNOSDB_OPTS >>$STDOUT 2>>$STDERR &
    else
        local CMD="$DAEMON --pidfile $PIDFILE --config $CONFIG $CNOSDB_OPTS >>$STDOUT 2>>$STDERR &"
