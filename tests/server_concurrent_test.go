...
func TestConcurrentServer_WriteValues(t *testing.T) {
	...
	write := strings.Join([]string{
		fmt.Sprintf(`a%%[1]d val=22.33 %d`, mustParseTime(time.RFC3339Nano, "2000-01-01T00:00:00Z").UnixNano()),
		fmt.Sprintf(`b%%[1]d val=22.33 %d`, mustParseTime(time.RFC3339Nano, "2000-01-01T00:00:00Z").UnixNano()),
		fmt.Sprintf(`c%%[1]d val=22.33 %d`, mustParseTime(time.RFC3339Nano, "2000-01-01T00:00:00Z").UnixNano()),
		fmt.Sprintf(`d%%[1]d val=22.33 %d`, mustParseTime(time.RFC3339Nano, "2000-01-01T00:00:00Z").UnixNano()),
	}, "\n")
...
func TestConcurrentServer_TagValues(t *testing.T) {
	...
	write := strings.Join([]string{
		fmt.Sprintf(`air,station=XiaoMaiDaoA,region=uswest temperature=22.33 %d`, mustParseTime(time.RFC3339Nano, "2000-01-01T00:00:00Z").UnixNano()),
		fmt.Sprintf(`air,station=XiaoMaiDaoA,region=useast temperature=22.33 %d`, mustParseTime(time.RFC3339Nano, "2000-01-01T00:00:00Z").UnixNano()),
		fmt.Sprintf(`air,station=XiaoMaiDaoA,region=ussouth temperature=22.33 %d`, mustParseTime(time.RFC3339Nano, "2000-01-01T00:00:00Z").UnixNano()),
		fmt.Sprintf(`air,station=XiaoMaiDaoA,region=usnorth temperature=22.33 %d`, mustParseTime(time.RFC3339Nano, "2000-01-01T00:00:00Z").UnixNano()),
	}, "\n")
	...
	stmt, err := cnosql.ParseStatement(`SHOW TAG VALUES WITH KEY = "region"`)
	if err != nil {
		t.Fatal(err)
	}
	...
	cond := rewrite.(*cnosql.ShowTagValuesStatement).Condition
	var f2 = func() {
