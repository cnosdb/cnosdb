...
use meta::meta_cluster_command::backup::backup;
use meta::meta_cluster_command::export::export;
...
enum Commands {
    ...
    /// Backup the cluster’s meta resources in SQL format to a file
    Backup {
    ...
    /// Export the cluster's meta resources to a file
    Export {
        /// Address of a cluster node
        #[arg(long)]
        bind: String,
...
async fn main() {
    ...
    match cli.command {
        ...
        Some(Commands::Backup { bind, file }) => {
            if let Err(e) = backup(&bind, &file).await {
        ...
        Some(Commands::Export { bind, file }) => {
            if let Err(e) = export(&bind, &file).await {
