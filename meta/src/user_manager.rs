...
pub trait UserManager: Send + Sync + Debug {
    // user
    fn create_user(&self, name: String, options: UserOptions, is_admin: bool) -> Result<Oid>;
    ...
    fn alter_user(&self, name: &str, options: UserOptions) -> Result<()>;
    fn drop_user(&self, name: &str) -> Result<bool>;
    fn rename_user(&self, old_name: &str, new_name: String) -> Result<()>;
}
...
impl UserManager for UserManagerMock {
    ...
    fn alter_user(&self, user_id: &str, options: UserOptions) -> Result<()> {
    ...
    fn rename_user(&self, user_id: &str, new_name: String) -> Result<()> {
