...
func Test_Data_CreateRetentionPolicy(t *testing.T) {
	...
	if err == nil || err != meta.ErrRetentionPolicyExists {
		t.Fatalf("unexpected error.  got: %v, exp: %s", err, meta.ErrRetentionPolicyExists)
	}
...
func TestData_SetPrivilege(t *testing.T) {
	...
	if got, exp := data.SetPrivilege("not a user", "db0", cnosql.AllPrivileges), meta.ErrUserNotFound; got != exp {
		t.Fatalf("got %v, expected %v", got, exp)
	}
