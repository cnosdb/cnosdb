...
fn test_write_upper_req() -> Vec<u8> {
    let sample = Sample {
        value: 1.1,
        timestamp: 1686819776617,
    };
...
fn test_read_upper_resp() -> ReadResponse {
    let sample = Sample {
        value: 1.1,
        timestamp: 1686819776617,
    };
...
fn test_prom() {
    ...
    let body = "drop table if exists TEST_upper;";
    check_response!(client.post("http://127.0.0.1:8902/api/v1/sql?db=public", body));
    ...
    // read upper tablename data
    let body = test_read_upper_req();
    let resp = client
        .request_with_auth(
            Method::POST,
            "http://127.0.0.1:8902/api/v1/prom/read?db=public",
    ...
    let resp: ReadResponse = deserialize(&resp.bytes().unwrap());
    assert_eq!(resp, test_read_upper_resp());
}
