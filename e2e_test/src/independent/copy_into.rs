...
fn wide_table_copy_in() {
    ...
    let workspace_dir: String = get_workspace_dir()
        ...
        .unwrap()
        .to_string();
    let data_cp_in_sql = gen_static_str(format!(
        "COPY INTO \"t1\" FROM '{}/e2e_test/test_data/' FILE_FORMAT = (TYPE = 'PARQUET');",
        workspace_dir
    ));

    let executor = E2eExecutor::new_singleton(
        "copy_into_tests",
        "wide_table_test",
        crate::cluster_def::one_data(1),
    );
