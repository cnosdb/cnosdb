...
use std::{io::{BufRead, BufReader}, path::{Path, PathBuf}, process::{Child, Command, Stdio}};

use crate::{cluster_def, utils::{get_workspace_dir, kill_all, run_singleton, Client, CnosdbDataTestHelper}};
...
fn new_server(test_dir: &str)->CnosdbDataTestHelper{
    ...
    let data: CnosdbDataTestHelper = run_singleton(
        test_dir, 
        data_node_def, 
        false, 
        true
...
fn simple_test(){
    ...
    //bug test for 1533
    {
        ...
        .post(url, "select t1,f1 from test;")
        .unwrap();
        ...
        let resp = server.client
        .post("http://127.0.0.1:8902/api/v1/write?db=public", "test,t2=t22,f2=2 1642176000000000000")
        .unwrap();
    ...
    // bug test for #1311 #1459
    {
        let url = "http://127.0.0.1:8902/api/v1/sql?db=public";
        let resp = server.client
        .post(url, "CREATE TENANT tenant_a;")
        .unwrap();

        assert_response_is_ok!(resp);

        let resp = server.client
        .post(url, "CREATE USER user_a;")
        .unwrap();
    ...
    }
    {
        ...
        let resp = server.client
        .post(url, "drop tenant tenant_a;")
        .unwrap();

        assert_response_is_ok!(resp);

        let resp = server.client
        .post(url, "drop user user_a;")
        .unwrap();
    ...
    }
    {
        ...
        let resp = client
        .post(url, "select * from db1.air_a;")
        .unwrap();
