...
impl Engine for TsKv {
    async fn open_tsfamily(
        ...
        if let Some(vnode) = self.version_set.read().await.get_vnode(vnode_id).cloned() {
            return Ok(vnode);
        }

        let owner = make_owner(tenant, db_name);
        let summary_file = file_utils::make_tsfamily_summary_file(
            self.ctx.options.storage.ts_family_dir(&owner, vnode_id),
        );
        ...
        let (ts_family, ts_index) = {
            let mut db = database.write().await;
            (
                db.get_tsfamily_or_create(vnode_id, summary.clone()).await?,
                db.get_tsindex_or_create(vnode_id).await?,
            )
        };

        let flush_job = FlushJob::new(self.ctx.clone(), summary.clone());
        ...
        self.version_set
            .write()
            .await
            .add_vnode(vnode_id, vnode.clone());
