...
pub fn decode_be_u128(key: &[u8]) -> u128 {
    unsafe { u128::from_le_bytes(*(key as *const _ as *const [u8; 16])) }
}

pub fn decode_be_i64(key: &[u8]) -> i64 {
    unsafe { i64::from_le_bytes(*(key as *const _ as *const [u8; 8])) }
}

pub fn decode_be_f64(key: &[u8]) -> f64 {
    unsafe { f64::from_le_bytes(*(key as *const _ as *const [u8; 8])) }
}

pub fn decode_be_bool(key: &[u8]) -> bool {
    let ret = false;
    let val = unsafe { u8::from_le_bytes(*(key as *const _ as *const [u8; 1])) };
    if val > 0 {
        return true;
    }
    ret
