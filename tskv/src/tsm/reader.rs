...
use crate::tsm::block_cache::DataBlockCache;
use crate::tsm::codec::{
...
impl TsmReader {
    ...
    pub async fn get_data_block(&self, block_meta: &BlockMeta) -> Result<DataBlock> {
        match self.cache.as_ref() {
            Some(cache) => {
                let blk = cache
                    .get_data_block_or_read(block_meta, self.reader.clone())
                    .await?;
                match self
                    .tombstone
                    .read()
