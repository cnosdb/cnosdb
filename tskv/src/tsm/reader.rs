...
impl IndexFile {
    pub async fn open(reader: Arc<DmaFile>) -> ReadTsmResult<Self> {
...
pub async fn load_index(reader: Arc<DmaFile>) -> ReadTsmResult<Index> {
    let len = reader.len();
...
impl IndexReader {
    pub async fn open(reader: Arc<DmaFile>) -> Result<Self> {
