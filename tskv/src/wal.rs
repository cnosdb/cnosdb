...
pub enum WalTask {
    Write { points: Vec<u8>, cb: oneshot::Sender<WalResult<()>> },
}
...
impl WalFileManager {
    pub fn new(config: kv_option::WalConfig) -> Self {
        ...
        WalFileManager { config, current_dir_path, current_file: WalFile { id: seq, file, size } }
    }
    ...
    pub async fn write(&mut self, typ: WalEntryType, data: &[u8]) -> WalResult<()> {
        ...
        let writer = &mut self.current_file;
        {
            ...
            writer.file
                  ...
                  .and_then(|size| {
                      pos += size as u64;
                      if self.config.sync { writer.file.sync_all(FileSync::Soft) } else { Ok(()) }
                  })
                  .map_err(|err| WalError::FailedWithStdIO { source: err })?;

            // TODO codes below may produce "future cannot be sent between threads safely" in
            // `kvcore.rs`." self.file_manager
...
pub fn reader<'a>(f: File) -> WalReader<'a> {
    WalReader { cursor: f.into_cursor(), phantom: PhantomData }
}
...
impl<'a> WalReader<'_> {
    ...
    pub fn next_wal_entry(&mut self) -> Option<WalEntryBlock> {
        ...
        let inner_block =
            WalEntryBlockInner { typ: typ.into(), crc, len: read_bytes as u32, buf: buf.to_vec() };
...
mod test {
    ...
    impl crate::wal::WalEntryBlock {
        ...
        pub fn new_delete_range(bytes: &[u8]) -> Self {
            Self::DeleteRange(WalEntryBlockInner::from_bytes(WalEntryType::DeleteRange, bytes))
        }
    }
    impl WalEntryBlockInner {
        pub fn from_bytes(typ: WalEntryType, bytes: &[u8]) -> Self {
            Self { typ, crc: crc32fast::hash(bytes), len: bytes.len() as u32, buf: bytes.into() }
        }
    ...
    fn random_delete_wal_entry<'a>(_fbb: &mut flatbuffers::FlatBufferBuilder<'a>)
        ...
        models::ColumnKeys::create(fbb, &models::ColumnKeysArgs { column_keys: Some(vec) })
    }
    ...
    fn random_delete_range_wal_entry<'a>(_fbb: &mut flatbuffers::FlatBufferBuilder<'a>)
        ...
        models::ColumnKeysWithRange::create(fbb,
                                            &models::ColumnKeysWithRangeArgs { column_keys:
                                                                                   Some(vec),
                                                                               min: 1,
                                                                               max: 100 })
    }
        ...
        for i in 0..10 {
            ...
            match entry {
                ...
                WalEntryBlock::DeleteRange(block) => {
                    ...
                    mgr.write(WalEntryType::DeleteRange, &block.buf).await.unwrap();
                },
                _ => {},
            };
        }
    ...
    fn test_read_entry() {
        ...
        for wal_file in wal_files {
            let file = file_manager::get_file_manager().open_file(mgr.current_dir_path
                                                                     .join(wal_file))
                                                       .unwrap();
