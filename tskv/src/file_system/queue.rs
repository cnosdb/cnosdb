...
use crate::error::{IOSnafu, TskvResult};
...
impl Queue {
    ...
    pub async fn write_bytes(&mut self, data: &[u8]) -> TskvResult<()> {
        ...
        self.write_file.write_all(data).await.context(IOSnafu)?;
        ...
        Block: DataBlock,
    {
        if let Err(e) = block.read(&mut self.read_file).await {
            let seek_cur = self
                ...
                .await
                .context(IOSnafu)?;
            let file_size = self.read_file.metadata().await.context(IOSnafu)?.len();
    ...
    pub async fn commit(&mut self) -> TskvResult<()> {
        let seek_cur = self
            ...
            .await
            .context(IOSnafu)?;
        Queue::write_offset(&mut self.read_file, seek_cur)
            .await
            .context(IOSnafu)?;
    ...
    pub async fn close(&mut self) -> TskvResult<()> {
        self.write_file.flush().await.context(IOSnafu)?;
    ...
    pub async fn size(&mut self) -> TskvResult<u64> {
        ...
        loop {
            ...
            let offset = Queue::read_offset(&mut file).await.context(IOSnafu)?;
            let file_size = file.metadata().await.context(IOSnafu)?.len();
    ...
    async fn open_write_file(file_name: impl AsRef<Path>) -> TskvResult<(File, u64)> {
        ...
        let mut file_size = file.metadata().await.context(IOSnafu)?.len();
        if file_size == 0 {
            ...
            file.write_all(&header_buf).await.context(IOSnafu)?;
    ...
    async fn open_read_file(file_name: impl AsRef<Path>) -> TskvResult<(File, u64)> {
        ...
        file.seek(SeekFrom::Start(offset)).await.context(IOSnafu)?;
