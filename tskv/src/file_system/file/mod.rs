    ...
    T: Send + 'static,
{
    ...
    let f: Box<dyn FnOnce() -> Result<T> + Send + 'static> = Box::new(f);
    match Fut(Some(spawn_blocking(f))).await {
        Ok(res) => res,
        Err(e) => Err(Error::new(
            ErrorKind::Other,
...
mod test {
    ...
    async fn test_drop_futures() {
        ...
        write_file.truncate(3 * 1024 * 1024 * 1024).await.unwrap();

        let token = CancellationToken::new();
        let cloned_token = token.clone();
        async fn test(file_system: LocalFileSystem, path: &Path) -> usize {
            let mut pos = 0_usize;
            let mut buf = BufWrapper {
                buf: vec![0_u8; 3 * 1024 * 1024 * 1024],
            };
            let read_file = file_system.open_file_reader(path).await.unwrap();
            while pos < 3 * 1024 * 1024 * 1024 {
                let read_size = read_file.read_at(pos, &mut buf.buf).await.unwrap();
                pos += read_size;
            }
            0
        }
